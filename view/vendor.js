...
vendor.post('/add', (req, res) => {
    ...
    let sql = `INSERT INTO VENDORS (${columns}) VALUES (${placeholders})`;
...
vendor.delete('/delete', (req, res) => {
    res.set('content-type', 'application/json');
    let sql = 'DELETE FROM VENDORS WHERE vendorId =?';
    let id = req.body.vendorId;
...
vendor.post('/update', (req, res) => {
    res.set('content-type', 'application/json');
    let query = 'UPDATE VENDORS SET ';
    const updates = []
    const values = []

    if (typeof req.body.vendorName !== 'undefined') {
        updates.push(`vendorName = ?`);
        values.push(req.body.vendorName);
    }
    if (typeof req.body.vendorWebsiteUrl !== 'undefined') {
        updates.push(`vendorWebsiteUrl = ?`);
        values.push(req.body.vendorWebsiteUrl);
    }
  
    let sql = query += updates.join(', ') + ' WHERE vendorId = ?';
    let id = req.body.vendorId;
