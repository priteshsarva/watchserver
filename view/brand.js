...
brand.post('/add', (req, res) => {
    ...
    let sql = `INSERT INTO BRAND (${columns}) VALUES (${placeholders})`;
...
brand.delete('/delete', (req, res) => {
    res.set('content-type', 'application/json');
    let sql = 'DELETE FROM BRAND WHERE brandId =?';
...
brand.post('/update', (req, res) => {
    ...
    if (typeof req.body.brandName !== 'undefined') {
        ...
        values.push(req.body.brandName);
    }
    if (typeof req.body.brandWebsiteUrl !== 'undefined') {
        updates.push(`brandWebsiteUrl = ?`);
        values.push(req.body.brandWebsiteUrl);
    }
  
    let sql = query += updates.join(', ') + ' WHERE brandId = ?';
    let id = req.body.brandId;
